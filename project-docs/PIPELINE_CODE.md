# CHIMERA Betfair Dataflow Pipeline - Complete Code Reference

## Overview

This document contains the complete, working pipeline code for processing Betfair BASIC tier 2024 data.
The pipeline successfully processes Steps 1-6 (read, decompress, parse, normalize). It failed at Step 7
(BigQuery write) due to schema mismatch - NOT a code issue.
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        market_snapshots = (
            ...
            | 'Write Market Snapshots' >> beam.io.WriteToBigQuery(
                ...
                create_disposition=beam.io.BigQueryDisposition.CREATE_IF_NEEDED,  # Changed from CREATE_NEVER
            )
        )
...
## File: deployment_command.sh

```bash
#!/bin/bash
# Betfair Dataflow Pipeline Deployment Script

# Set variables
PROJECT_ID="betfair-data-explorer"
REGION="us-central1"
TEMP_BUCKET="gs://betfair-data-explorer-temp"
...
## File: create_bigquery_tables.sh

```bash
#!/bin/bash
# Create BigQuery tables with correct schema
# Run this BEFORE the pipeline if tables don't exist

PROJECT_ID="betfair-data-explorer"
DATASET="chimera_horses_2024"

# Delete existing tables (if they have wrong schema)
echo "Removing old tables..."
bq rm -f -t ${PROJECT_ID}:${DATASET}.market_snapshots
bq rm -f -t ${PROJECT_ID}:${DATASET}.horse_race_participations

# Create market_snapshots table
echo "Creating market_snapshots table..."
bq mk --table \
    --description "Market snapshots from Betfair MCM data" \
    --time_partitioning_field race_date \
    --time_partitioning_type DAY \
    --clustering_fields horse_id,market_id,snapshot_time \
    ${PROJECT_ID}:${DATASET}.market_snapshots \
    race_date:DATE,horse_id:STRING,horse_name:STRING,race_id:STRING,market_id:STRING,runner_id:INTEGER,snapshot_time:TIMESTAMP,ltp:FLOAT64,back_price:FLOAT64,lay_price:FLOAT64,back_volume:FLOAT64,lay_volume:FLOAT64,total_matched_volume:FLOAT64,implied_probability:FLOAT64,track_name:STRING,race_name:STRING,market_type:STRING,country_code:STRING,event_id:STRING,ingest_timestamp:TIMESTAMP,source_file:STRING

# Create horse_race_participations table
echo "Creating horse_race_participations table..."
bq mk --table \
    --description "Horse race participations from Betfair MCM data" \
    --time_partitioning_field race_date \
    --time_partitioning_type DAY \
    --clustering_fields horse_id,track_name,market_id \
    ${PROJECT_ID}:${DATASET}.horse_race_participations \
    race_date:DATE,horse_id:STRING,horse_name:STRING,race_id:STRING,market_id:STRING,runner_id:INTEGER,track_name:STRING,race_name:STRING,market_type:STRING,event_id:STRING,country_code:STRING,closing_ltp:FLOAT64,closing_implied_prob:FLOAT64,ingest_timestamp:TIMESTAMP,source_file:STRING

echo "Tables created successfully!"
echo ""
echo "Verify with:"
echo "  bq ls ${PROJECT_ID}:${DATASET}"
echo "  bq show --schema ${PROJECT_ID}:${DATASET}.market_snapshots"
echo "  bq show --schema ${PROJECT_ID}:${DATASET}.horse_race_participations"
```
...
```bash
#!/bin/bash
# Test pipeline locally with DirectRunner on a small subset

# Create a test file pattern (limit to one month)
python3 betfair_dataflow_pipeline.py \
    --runner DirectRunner

# Note: For local testing, modify GCS_PREFIX to a specific month:
# GCS_PREFIX = "BASIC/2024/Jan"
# And add a file limit:
# | beam.combiners.Sample.FixedSizeGlobally(1000)
```

---

## Betfair MCM Data Format Reference

### Sample MCM Message Structure
    ...
    "mc": [
        {
            ...
            "marketDefinition": {
                "eventId": "31234567",
                "eventName": "Kempton 2:10",
                "name": "2m Hcap Hdle",
                "marketTime": "2024-01-01T14:10:00.000Z",
                "marketType": "WIN",
                "countryCode": "GB",
                "runners": [
                    {"id": 12345678, "name": "Horse Name 1"},
                    {"id": 23456789, "name": "Horse Name 2"}
                ]
            },
            "rc": [
                {
...
| MCM Field | Description | Output Field |
|-----------|-------------|--------------|
| `op` | Operation type (mcm = market change) | Filter only |
| `pt` | Publish timestamp (milliseconds) | `snapshot_time` |
| `mc[].id` | Market ID (e.g., "1.123456789") | `market_id` |
| `mc[].marketDefinition.eventId` | Event ID | `event_id` |
| `mc[].marketDefinition.eventName` | Track/venue name | `track_name` |
| `mc[].marketDefinition.name` | Race name | `race_name` |
| `mc[].marketDefinition.marketTime` | Scheduled race time | `race_date` |
| `mc[].marketDefinition.marketType` | WIN, PLACE, etc. | `market_type` |
| `mc[].marketDefinition.countryCode` | GB, IE, US, etc. | `country_code` |
| `mc[].rc[].id` | Runner/selection ID | `runner_id` |
| `mc[].rc[].ltp` | Last traded price | `ltp`, `closing_ltp` |
| `mc[].rc[].bp` | Best back price | `back_price` |
| `mc[].rc[].lp` | Best lay price | `lay_price` |
| `mc[].rc[].bv` | Back volume | `back_volume` |
| `mc[].rc[].lv` | Lay volume | `lay_volume` |
| `mc[].rc[].tv` | Total matched volume | `total_matched_volume` |

---

## Notes on Working Code

1. **Decompression is automatic** - Beam's TextIO handles .bz2 based on file extension
2. **Error handling** - Malformed JSON lines are logged and skipped
3. **Horse name fallback** - Uses runner ID if name not available
4. **Implied probability** - Calculated as `1/ltp` when ltp > 0
5. **Composite IDs** - `horse_id` = `{name}_{runner_id}`, `race_id` = `{event_id}_{market_id}`

## Known Limitations

1. **No deduplication** - Race participations outputs all snapshots, not just closing prices
2. **source_file is None** - File attribution not implemented yet
3. **No validation** - Invalid dates/prices pass through unchecked
